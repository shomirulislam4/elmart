...
/*============================ Single Product Page ============================*/
    ...
    $(function() {
        $('.elt__quantity__widget').each(function() {
            ...
            const $increaseBtn = $(this).find('.elt__quantity__increase');
            const $display = $(this).find('.elt__quantity__display');
            const $priceTd = $(this).closest('tr').find('.elt__initprice');
            const $totalTd = $(this).closest('tr').find('.elt__subtotal');
            const price = parseFloat($priceTd.text().replace('$', ''));
    
            const formatQuantity = (num) => num.toString().padStart(2, '0');
            ...
            $decreaseBtn.on('click', () => {
                ...
                if (quantity > 1) {
                    quantity--;
                    $display.text(formatQuantity(quantity));
                    updateTotal();
                }
            });
            ...
            // Ensure initial display is formatted
